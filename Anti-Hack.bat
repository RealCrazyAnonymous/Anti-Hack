@echo off
title Anti-Hack Security Hardener for Windows 11 - Made by Crazy Anonymous
color 0A
echo ====================================================================
echo     Anti-Hack Batch Script for Windows 11 - Made by Crazy Anonymous
echo ====================================================================
echo.
...
:: Check OS Version (Windows 11 only - build 22000+)
echo Checking OS version...
for /f "tokens=*" %%i in ('systeminfo ^| findstr /B /C:"OS Name" /C:"OS Version"') do (
    set OS_INFO=%%i
)
echo %OS_INFO% | findstr /C:"Microsoft Windows 11" >nul
if %errorLevel% neq 0 (
    echo ERROR: This script is designed ONLY for Windows 11.
...
:: Disable Remote Desktop (if enabled)
...
echo - Enable BitLocker (if on compatible hardware).
